#!/usr/bin/env python3
"""
Mubawab — MongoDB (locations) -> PostgreSQL Sync + Archive Checker
...
log = logging.getLogger("mubawab-sync")
...
def ensure_schema(pg_conn):
    ...
    pg_conn.commit()
    log.info("✅ Table mubawab_listings ensured")
...
def get_existing_ids(pg_conn):
    cur = pg_conn.cursor()
    cur.execute(f"SELECT ad_id FROM {PG_TABLE} WHERE ad_id IS NOT NULL")
...
def sync(mongo_col, pg_conn):
    ...
    for doc in mongo_col.find({}, batch_size=BATCH_SIZE):
        ...
        try:
            ...
            stats["errors"] += 1
            if stats["errors"] <= 5:
                log.warning(f"  Row build error (ad_id={doc_id}): {e}")
...
def archive_listing(pg_conn, unique_val, reason="listing_removed"):
    ...
    try:
        ...
        cur.execute(f"""
            ...
            SELECT {cols_str}, NOW(), %s FROM {PG_TABLE} WHERE ad_id = %s
        """, (reason, unique_val))
        cur.execute(f"DELETE FROM {PG_TABLE} WHERE ad_id = %s", (unique_val,))
        pg_conn.commit()
        return True
    except Exception as e:
        pg_conn.rollback()
...
def archive_check(pg_conn):
    ...
    cur.execute(f"SELECT ad_id, url FROM {PG_TABLE} WHERE url IS NOT NULL")
    rows = cur.fetchall()
    ...
    for i, row in enumerate(rows):
        ...
        if alive:
            ...
            if archive_listing(pg_conn, row["ad_id"]):
                stats["archived"] += 1
                log.info(f"  📦 Archived: {row['ad_id']}")
            else:
                stats["errors"] += 1
        if (i + 1) % 50 == 0:
...
def run_cycle(mongo_col, pg_conn, do_sync=True, do_archive=True):
    ...
    log.info(f"\n{'='*60}")
    log.info(f"[🇹🇳 MUBAWAB] CYCLE START: {now}")
    log.info(f"{'='*60}")
...
def main():
    parser = argparse.ArgumentParser(description="Mubawab Sync + Archive")
